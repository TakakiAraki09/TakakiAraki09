...
const convertToUserAnimeListItemEntity = (item: Daum): UserAnimeListItemEntity => {
  return {
    malId: item.node.id,
    ...
    updatedAt: item.list_status.updated_at ? new Date(item.list_status.updated_at) : null,
    startDate: item.list_status.start_date ? new Date(item.list_status.start_date) : null,
    finishDate: item.list_status.finish_date ? new Date(item.list_status.finish_date) : null,
  }
}
...
export const userAnimeList = async ({
  ...
  const root = await createMyAnimeListAPI<Root>(`/v2/users/${userName}/animelist`)(url => {
