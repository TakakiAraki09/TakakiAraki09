...
export const exportLibrary = async ({
  ...
  // DBから全データを取得
  const [contents, contentAnimes, contentStates, contentNews] =
    await Promise.all([
      findAllContent(),
      findAllContentAnime(),
      findAllContentState(),
  ...
  console.log(
    `Found ${contents.length} contents, ${contentAnimes.length} content animes, ${contentStates.length} content states, and ${contentNews.length} content news`,
  );
  ...
  writeFileSync(
    contentAnimePath,
    JSON.stringify(contentAnimeEntities, null, 2),
    "utf-8",
  );
  writeFileSync(
    contentStatePath,
    JSON.stringify(contentStateEntities, null, 2),
    "utf-8",
  );
  writeFileSync(
    contentNewsPath,
    JSON.stringify(contentNewsEntities, null, 2),
    "utf-8",
  );

  console.log(
    `Exported to ${contentPath}, ${contentAnimePath}, ${contentStatePath}, and ${contentNewsPath}q`,
  );
