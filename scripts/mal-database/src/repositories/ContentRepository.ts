...
export const findAllContent = async (): Promise<Content[]> => {
  return await db.selectFrom("content").selectAll().execute();
};

export const findAllContentAnime = async (): Promise<ContentAnime[]> => {
  return await db.selectFrom("content_anime").selectAll().execute();
};

export const findAllContentState = async (): Promise<ContentState[]> => {
  return await db.selectFrom("content_state").selectAll().execute();
};
...
export const createContentNews = async (
  content: NewContent,
  contentNews: NewContentNews,
) => {
  const createdContent = await db
    .insertInto("content")
    .values(content)
...
export const findAllContentNews = async (): Promise<ContentNews[]> => {
  return await db.selectFrom("content_news").selectAll().execute();
};

export const upsertContentNews = async (
  content: NewContent,
  contentNews: NewContentNews,
) => {
